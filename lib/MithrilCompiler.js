...
var Compiler = function (node, options) {
	return {
		...
		visitAttributes: function( node, bindingPath, wasArray, inArray, arrayBinding ){
			...
			if( options.context && (arrayBinding || attributeBinding || styleBinding || visibilityBinding || valueBinding || fieldBinding) ){
				...
				self.code.push( 'config: createConfig( ' + rootReferer + ', \''+ bindingPath +'\', { ' + itemSpec + visiSpec +  attSpec + styleSpec + ' V: ctrl._validation } ),' );
			}
