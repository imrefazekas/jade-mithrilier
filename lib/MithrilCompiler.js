	...
	var compiler = {
		...
		visitAttributes: function( node, bindingPath, inArray, arrayBinding ){
			...
			if( options.context && (arrayBinding || valueBinding || fieldBinding || memberBinding || tapBinding || visibilityBinding || displayBinding || attributeBinding || styleBinding || htmlBinding || eventBindings.length>0 ) ){
				...
				self.code.push( 'config: createConfig( ctrl, viewName, modelName, context, ' + rootReferer + ', \''+ bindingPath +'\', { ' + itemSpec + memberSpec + visiSpec + displaySpec +  attSpec + styleSpec + htmlSpec + tapSpec + eventBindingCode + ' V: ctrl._validation, clearElement: context.clearElement, invalidElement: context.invalidElement, validElement: context.validElement } ),' );
			}
