...
var Compiler = function (node, options) {
	return {
		...
		visitAttributes: function( node, bindingPath, wasArray, inArray, arrayBinding ){
			...
			if( options.context && (arrayBinding || valueBinding || fieldBinding  || visibilityBinding || displayBinding || attributeBinding || styleBinding || htmlBinding || eventBindings.length>0 ) ){
				...
				self.code.push( 'config: createConfig( context, ' + rootReferer + ', \''+ bindingPath +'\', { ' + itemSpec + visiSpec + displaySpec +  attSpec + styleSpec + htmlSpec + eventBindingCode + ' V: ctrl._validation, clearElement: context.clearElement, invalidElement: context.invalidElement, validElement: context.validElement } ),' );
			}
		...
		compile: function () {
			...
			if( options.context ){
				...
				self.code.push( 'context.emit( \'init\' + name + \'Context\', context[name] );' );
			}
