...
var Compiler = function (node, options) {
	var compiler = {
		...
		visitAttributes: function ( node, bindingPath, inArray, arrayBinding ) {
			...
			var valuationBinding = cleanString( getBindingAttribute( node, 'data-valuate' ) )
			...
			var propertyBinding = cleanString( getBindingAttribute( node, 'data-prop' ) )
			var attributeBinding = cleanString( getBindingAttribute( node, 'data-attr' ) )
			var classEnableBinding = cleanString( getBindingAttribute( node, 'data-class-enable' ) )
			...
			if ( options.context && (arrayBinding || valueBinding || fieldBinding || valuationBinding || memberBinding || tapBinding || visibilityBinding || displayBinding || propertyBinding || attributeBinding || classEnableBinding || attributeEnableBinding || styleBinding || enableBinding || selectBinding || htmlBinding || eventBindings.length > 0 ) ) {
				var itemSpec = inArray ? 'array: array, item: item, index: index,' : ' '
				var valuationSpec = valuationBinding ? '\'data-valuate\': ' + stringify( valuationBinding ) + ',' : ''
				var visiSpec = visibilityBinding ? '\'data-visible\': ' + stringify( visibilityBinding ) + ',' : ''
				var displaySpec = displayBinding ? '\'data-display\': ' + stringify( displayBinding ) + ',' : ''
				var propSpec = propertyBinding ? '\'data-prop\': ' + stringify( propertyBinding ) + ',' : ''
				var attSpec = attributeBinding ? '\'data-attr\': ' + stringify( attributeBinding ) + ',' : ''
				var classEnableSpec = classEnableBinding ? '\'data-class-enable\': ' + stringify( classEnableBinding ) + ',' : ''
				var attEnableSpec = attributeEnableBinding ? '\'data-attr-enable\': ' + stringify( attributeEnableBinding ) + ',' : ''
				var styleSpec = styleBinding ? '\'data-style\': ' + stringify( styleBinding ) + ',' : ''
				var selectSpec = selectBinding ? '\'data-select\': ' + stringify( selectBinding ) + ',' : ''
				var enableSpec = enableBinding ? '\'data-enable\': ' + stringify( enableBinding ) + ',' : ''
				var htmlSpec = htmlBinding ? '\'data-html\': ' + stringify( htmlBinding ) + ',' : ''
				var tapSpec = tapBinding ? '\'data-tap\': ' + stringify( tapBinding ) + ',' : ''
				...
				self.code.push( 'config: createConfig( ctrl, controlOptions, viewName, modelName, context, ' + rootReferer + ', \'' + bindingPath + '\', { ' + itemSpec + valuationSpec + memberSpec + visiSpec + displaySpec + propSpec + attSpec + classEnableSpec + attEnableSpec + styleSpec + selectSpec + enableSpec + htmlSpec + tapSpec + eventBindingCode + ' V: ctrl._validation, clearElement: context.clearElement, invalidElement: context.invalidElement, validElement: context.validElement } ),' )
			}
