...
var Compiler = function (node, options) {
	var compiler = {
		...
		visitAttributes: function ( node, bindingPath, inArray, arrayBinding ) {
			...
			if ( options.context && (arrayBinding || valueBinding || fieldBinding || valuationBinding || memberBinding || tapBinding || visibilityBinding || displayBinding || attributeBinding || attributeEnableBinding || styleBinding || enableBinding || selectBinding || htmlBinding || eventBindings.length > 0 ) ) {
				...
				self.code.push( 'config: createConfig( ctrl, controlOptions, viewName, modelName, context, ' + rootReferer + ', \'' + bindingPath + '\', { ' + itemSpec + valuationSpec + memberSpec + visiSpec + displaySpec + attSpec + attEnableSpec + styleSpec + selectSpec + enableSpec + htmlSpec + tapSpec + eventBindingCode + ' V: ctrl._validation, clearElement: context.clearElement, invalidElement: context.invalidElement, validElement: context.validElement } ),' )
			}
