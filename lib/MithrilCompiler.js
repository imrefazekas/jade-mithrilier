...
var Compiler = function (node, options) {
	return {
		...
		visitAttributes: function( node, bindingPath, wasArray, inArray, arrayBinding ){
			...
			/*if( textElements.indexOf( node.name ) > -1 ){
			...
			if( options.context && (arrayBinding || attributeBinding || visibilityBinding || valueBinding || fieldBinding) ){
				console.log( inArray, arrayBinding, attributeBinding, visibilityBinding, valueBinding, fieldBinding );
				...
				self.code.push( 'config: createConfig( ' + rootReferer + ', \''+ bindingPath +'\', { ' + itemSpec + visiSpec +  attSpec + ' V: ctrl._validation } ),' );
			}
