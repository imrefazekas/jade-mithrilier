	...
	return {
		...
		visitAttributes: function( node, bindingPath, wasArray, inArray, arrayBinding ){
			...
			if( options.context && (arrayBinding || valueBinding || fieldBinding  || tapBinding || visibilityBinding || displayBinding || attributeBinding || styleBinding || htmlBinding || eventBindings.length>0 ) ){
				...
				self.code.push( 'config: createConfig( viewName, modelName, context, ' + rootReferer + ', \''+ bindingPath +'\', { ' + itemSpec + visiSpec + displaySpec +  attSpec + styleSpec + htmlSpec + tapSpec + eventBindingCode + ' V: ctrl._validation, clearElement: context.clearElement, invalidElement: context.invalidElement, validElement: context.validElement } ),' );
			}
		...
		visitArrayBinding: function(node, bindingPath, wasArray, inArray, bindName, filterBinding){
			var self = this;
			...
			self.code.push(' ' + reference + '.' + binder + (filterBinding?'.filter( ctrl[modelName].'+filterBinding+' )':'') + '.map( function(item, index, array ) { return ');
		},
		visitTag: function( node, bindingPath, inArray ){
			...
			if( arrayBinding )
				self.visitArrayBinding( node, bindingPath, wasArray, inArray, arrayBinding, filterBinding );
		...
		compile: function () {
			...
			self.code.push( 'ctrl.dateFormat = envOptions.dateFormat;' );
			if( options.context ){
