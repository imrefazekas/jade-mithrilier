...
Compiler = function (node, options) {
	return {
		compile: function () {
			...
			needsMap = false;
			var getDataEachAttribute = function( node ){
				var types = _.map( _.filter( node.attrs, function(att){
					return att.name === 'data-each';
				} ), function( attr ){
					return attr.val;
				});
			...
			visitArgs = function (node) {
				...
				var eachBinder = getDataEachAttribute( node );
				ref = node.block.nodes;

				if( eachBinder )
					bufferExpression(' ctrl[ name ].' + eachBinder + '.map( function(item, index) { return ');
				...
				if( normalized['data-bind'] || normalized['data-value'] ){
					...
					if( normalized['data-bind'] ){
						...
						if( (node.name === 'input') && (type==='checkbox') ){
							bindings.push( 'onclick: m.withAttr(\'checked\', ctrl[ name ].' + bindName + ' ), checked: ctrl[ name ].'+bindName+'()' );
						}
