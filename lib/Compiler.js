...
Compiler = function (node, options) {
	return {
		compile: function () {
			...
			var bufferExpression, depth, prefix, postfix, needsMap, normalizeAttributes, normalizeClassExpressions, parts, pretty, seenDepth0, toPush, visit, visitArgs, visitAttributes, visitBlock, visitCode, visitEach, visitNodes, visitTag, visitText;
			...
			var visitBindings = function( node, normalized, bufferExpression ){
				var bindings = [];
				console.log( node, node.name, normalized );
				/*config: createConfig( ctrl[ name ], 'email', ctrl._validation ),
					class: "c_ds_blue hN text-center",
			...
			return prefix + parts.map(toPush).join('\n') + postfix;
		}
	};
};
