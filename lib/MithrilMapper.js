...
function mapToController( controller, root, object, model, value, arrayCreation ){
	...
	_.forEach( model, function(n, key){
		if( _.isString( n ) ){
			...
			controller['fill' + capName ] = function( _as, keepContent, context ) {
				m.startComputation();
				if( !keepContent )
				...
				if( context && context.emit )
					context.emit( 'filled' + capName, _as );
			};
			controller['addTo' + capName ] = function( _v, context ) {
				m.startComputation();
				var newItem = mapToController( controller, root, {}, n[0], _v, arrayCreation );
				object[ key ].push( newItem );
				m.endComputation();
				if (context && context.emit)
					context.emit( 'addedTo' + capName, newItem );
			};
			controller['removeFrom' + capName ] = function( index, context ) {
				m.startComputation();
				var oldData = object[ key ][index];
				object[ key ].splice( index, 1);
				m.endComputation();
				if( context && context.emit )
					context.emit( 'removedFrom' + capName, index, oldData );
			};
			controller['clean' + capName ] = function( index, context ) {
				m.startComputation();
				object[ key ].splice( 0, object[ key ].length );
				m.endComputation();
				if( context && context.emit )
					context.emit( 'cleaned' + capName );
			};
		}
